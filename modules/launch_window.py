...
class LaunchWindow(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        # main window widgets
        self.label_image_info = tk.Label(self, text="Select an image to view")
        self.button_browse_image = tk.Button(
            self, text="Browse", command=self.browse_image)
        self.button_load_image = tk.Button(
            self, text="Load image", command=self.launch_image_window)
    ...
    def browse_image(self):
        ...
        try:
            ...
            self.label_image_info.configure(
                text="Image file read: "+".../"+(file_path.split('/')[-1]))
            ...
            self.label_image_info.configure(
                text="Browse to select an image file")
            self.var_selected_image_path = ""
            self.button_load_image.pack_forget()

    def launch_image_window(self):
        self.new_window = tk.Toplevel(self.parent)
        var_title = "Andrew's Awesome Image Processor! - {}".format(
            self.var_selected_image_path)
        self.new_window.title(var_title)
        self.image_window = ImageWindow(
            self.new_window, self.var_selected_image_path)
